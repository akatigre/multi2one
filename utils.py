...
def splitS(s, style_space, style_names, fs3=None):
    styles = []
    idx = 0
    t_idx = 0
    
    for name, style in zip(style_names, style_space):
        num_channel = style.shape[-1]
        if fs3 is not None:
            total = fs3.shape[0]
            if ("torgb" not in name) and t_idx<total:
                styles.append(s[:, idx: idx+num_channel])
                t_idx += num_channel
            else:
                styles.append(torch.zeros_like(s[:, idx: idx+num_channel]))
        else:
            styles.append(s[:, idx: idx+num_channel])
        idx += num_channel
    return styles