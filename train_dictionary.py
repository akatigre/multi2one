...
class SClipLoss(torch.nn.Module):
    def __init__(self, args, singleS2C=None):
        ...
        latent = torch.load(args.latents_path, map_location='cpu').to(args.device)
        self.args = args
        if latent.shape[0]>1:
            self.latent = latent[0:1, :, :]
...
if __name__=="__main__":
    ...
    parser.add_argument('--alpha_lmbd',     type=float, default=0)
    ...
    clip_dirs = glob(f'stylespace/{args.dataset}/clip/LayerwiseClipAlpha{args.alpha}_ch*.npy')
    for cd in clip_dirs:
        ...
        multiS2C.append(t)
        d = torch.load(f'stylespace/{args.dataset}/delta/LayerwiseDelta_ch{num_c}.pt')
        deltas.append(d)
    ...
    y = torch.Tensor(np.load(f'dictionary/{args.dataset}/fs3.npy').astype(np.float32)).to(args.device) # Global Direction
    x = torch.zeros(y.shape).to(args.device)
    ...
    name = f"{'_'.join(sorted(channels))}_max_{args.alpha_lmbd}"
    for epoch in prog_bar:
        ...
        if (epoch)%1000==0 and best_loss>loss.item():
            best_loss = loss.item()
            file_name = f'dictionary/{args.dataset}/multi2one.pt'
